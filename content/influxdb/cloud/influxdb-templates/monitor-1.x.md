...
Use [InfluxDB Cloud](/influxdb/cloud/), the [InfluxDB 1.x Open Source (OSS) Metrics template](https://github.com/influxdata/community-templates/tree/master/monitoring_influxdb_1.x), and Telegraf to monitor one or more InfluxDB OSS instances.
...
Before you begin, make sure you have access to the following:
 ...
 - Command line access to a machine [running InfluxDB OSS 1.x](/influxdb/v2.0/install/) and permissions to install Telegraf on this machine
...
## Install the InfluxDB 1.x OSS Monitoring template
...
2. Click **Lookup Template**, and then click **Install Template**. InfluxDB Cloud imports the template, which includes the following resources:
   - Bucket `Telegraf`and 7d retention
   - 3 Labels `InfluxDB1.x`, `Solution`, and `Telegraf`
   - Telegraf Configuration `InfluxDB_1.x.conf`
...
By default, InfluxDB OSS 1.x has a `/metrics` endpoint available, which exports Prometheus-style system metrics.
